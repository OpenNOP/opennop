CC=gcc
PWD=$(shell pwd)
INCLUDE_DIR= $(PWD)/include
CFLAGS=-c -Wall -Wcast-align -Wcast-qual
libnfnetlink_LIBS=`pkg-config --libs libnfnetlink`
libnfnetlink_CPPFLAGS=`pkg-config --cflags libnfnetlink`
libnetfilter_queue_LIBS=`pkg-config --libs libnetfilter_queue`
libnetfilter_queue_CPPFLAGS=`pkg-config --cflags libnetfilter_queue`
libnl_LIBS=`pkg-config --libs libnl-1`
libnl_CPPFLAGS=`pkg-config --cflags libnl-1`
ncurses_LIBS=`pkg-config --libs ncurses`
ncurses_CPPFLAGS=`pkg-config --cflags ncurses`
OPENNOPDLDFLAGS=-lpthread -lcrypt $(libnfnetlink_LIBS) $(libnetfilter_queue_LIBS) $(libnl_LIBS)
OPENNOPLDFLAGS= -lpthread $(ncurses_LIBS)
CPPFLAGS=$(libnfnetlink_CPPFLAGS) $(libnetfilter_queue_CPPFLAGS) $(libnl_CPPFLAGS) $(ncurses_CPPFLAGS)
SOURCES=opennop/opennop opennopd/opennopd
OPENNOPD_OBJS=$(patsubst %.c,%.o,$(wildcard opennopd/*.c)) $(patsubst %.c,%.o,$(wildcard opennopd/subsystems/*.c)) $(patsubst %.c,%.o,$(wildcard opennopd/qlzc/*.c)) $(patsubst %.c,%.o,$(wildcard opennopd/libcli/*.c))
OPENNOP_OBJS=$(patsubst %.c,%.o,$(wildcard opennop/*.c)) $(patsubst %.c,%.o,$(wildcard opennop/subsystems/*.c))
OPENNOP=opennop
OPENNOPD=opennopd
DESTDIR?=/usr/local/bin

all: $(SOURCES)

opennop/opennop: $(OPENNOP_OBJS) $(OPENNOP)
	$(CC) $(CPPFLAGS) -I $(INCLUDE_DIR) $(OPENNOP_OBJS) -o $@ $(OPENNOPLDFLAGS)

opennopd/opennopd: $(OPENNOPD_OBJS) $(OPENNOPD)
	$(CC) $(CPPFLAGS) -I $(INCLUDE_DIR) $(OPENNOPD_OBJS) -o $@ $(OPENNOPDLDFLAGS)
	
.c.o:
	$(CC) $(CPPFLAGS) -I $(INCLUDE_DIR) $(CFLAGS) $< -o $@

clean:
	rm $(OPENNOP_OBJS)
	rm $(OPENNOPD_OBJS)
	rm $(SOURCES)
	
install:
	mkdir -p $(DESTDIR)
	install opennop/opennop $(DESTDIR)/opennop
	install opennopd/opennopd $(DESTDIR)/opennopd
