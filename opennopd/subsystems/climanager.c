...
void start_cli_server()
{
	...
	while(1){
		...
		if ( (client_fd = accept(server_fd, (struct sockaddr *)&client, &data_length )) == -1) {
		...
		while(!(finish) && (client_fd)) {
			...
			if (data_length) 
				finish = cli_process_message(client_fd, sock_buffer,data_length);
		}
